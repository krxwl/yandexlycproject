...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply(
            ...
            "песен на сегодня из чарта Яндекс.Музыки, для этого нужно нажать кнопку 🎵\n\n"
            "🧐 Распознать текст/песню из вашего голосового"
            " сообщения или аудиозаписи, для этого просто пришлите гс или аудио"
            "\n\n☀️ Вывести погоду из Вашего города, для этого нажмите кнопку ⛅️\n\n🚘 Вывести "
            "балл пробок на данный момент, для этого введите команду в формате /get_traffic "
            "<нужное место>\n\n📍 Найти ближайшую к Вам станцию метро, "
            "для этого просто пришлите мне свою геопозицию",
            reply_markup=keyboard)
    except Exception:
        global user
        user = User()
...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    await message.reply("Привет!\n\nМеня зовут yandexbot и я умею следующие вещи:\n\n🎸 Вывести топ 10 "
            ...
            " сообщения или аудиозаписи, для этого просто пришлите гс или аудио"
            "\n\n☀️ Вывести погоду из Вашего города, для этого нажмите кнопку ⛅️\n\n🚘 Вывести "
            "балл пробок на данный момент, для этого введите команду в формате:\n/get_traffic "
            "<нужное место>\n\n📍 Найти ближайшую к Вам станцию метро, "
            "для этого просто пришлите мне свою геопозицию",
                        reply_markup=keyboard)
...
async def send_top_of_the_yandex_music(message: types.Message):
    try:
        ...
        await message.reply('Извините, произошла непредвиденная ошибка, но мы уже пытаемся ее '
                            'починить')
...
async def send_welcome(message: types.Message):
    if message.location is not None:
        try:
            ...
            await message.reply(f'Ближайшая станция метро к Вам:\n{metro}')
            abc = geocode(metro)
            ...
            if response.content:
                await bot.send_photo(photo=InputFile('img.png'), chat_id=message.from_user.id)
                await message.reply('Вы отмечены  🏳️, ближайшая станция метро  🚩')
            else:
                await message.reply('Ошибка обработки запроса. Попробуйте позже')
        except Exception:
            await message.reply('Упс, произошла какая то ошибка')
        if os.path.isfile('img.png'):
            os.remove('img.png')
